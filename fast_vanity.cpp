...
void highSpeedSearchWorker(std::string lower_prefix, int thread_id) {
    ...
    size_t prefix_len = lower_prefix.length();
    ...
    while (!found) {
        ...
        for (uint16_t counter = 0; counter < 65535 && !found; ++counter) {
            ...
            // High-speed case-insensitive character verification loop
            bool match = true;
            for (size_t i = 0; i < prefix_len; ++i) {
                // address_buffer[i + 1] skips the network '1' prefix
                if (std::tolower(static_cast<unsigned char>(address_buffer[i + 1])) != lower_prefix[i]) {
                    match = false;
                    break;
                }
            }
            ...
            if (match) {
                ...
                if (found.compare_exchange_strong(expected, true)) {
                    ...
                    std::cout << "\n🎉 SUCCESS! Case-Insensitive Match Found by Worker " << thread_id << "\n";
...
int main() {
    ...
    // Normalize target prefix to lowercase for matching routines
    std::string lower_prefix = target_prefix;
    std::transform(lower_prefix.begin(), lower_prefix.end(), lower_prefix.begin(),
                   [](unsigned char c){ return std::tolower(c); });

    unsigned int threads = std::thread::hardware_concurrency();
    
    std::cout << "🚀 Starting Case-Insensitive Hashing Engine...\n";
    std::cout << "🎯 Target Pattern (Any Case): 1" << target_prefix << "\n";
    std::cout << "🧬 Difficulty Reduced By: 512x Faster!\n";
    ...
    for (unsigned int i = 0; i < threads; ++i) {
        worker_threads.push_back(std::thread(highSpeedSearchWorker, lower_prefix, i));
    }
